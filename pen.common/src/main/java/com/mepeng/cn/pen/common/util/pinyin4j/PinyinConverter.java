...
class PinyinConverter
{
     ...
     * @throws ChinesePinyinOutputFormatCombinationException
     */
    static String formatHanyuPinyin(String pinyinStr,
            ChinesePinyinOutputFormat outputFormat)
            throws ChinesePinyinOutputFormatCombinationException
    {
        if ((ChinesePinyinToneType.WITH_TONE_MARK == outputFormat.getToneType())
                && ((ChinesePinyinVCharType.WITH_V == outputFormat.getVCharType()) || (ChinesePinyinVCharType.WITH_U_AND_COLON == outputFormat.getVCharType())))
        {
            throw new ChinesePinyinOutputFormatCombinationException("tone marks cannot be added to v or u:");
        }

        if (ChinesePinyinToneType.WITHOUT_TONE == outputFormat.getToneType())
        {
            pinyinStr = pinyinStr.replaceAll("[1-5]", "");
        } else if (ChinesePinyinToneType.WITH_TONE_MARK == outputFormat.getToneType())
        {
            pinyinStr = pinyinStr.replaceAll("u:", "v");
            pinyinStr = convertToneNumber2ToneMark(pinyinStr);
        }

        if (ChinesePinyinVCharType.WITH_V == outputFormat.getVCharType())
        {
            pinyinStr = pinyinStr.replaceAll("u:", "v");
        } else if (ChinesePinyinVCharType.WITH_U_UNICODE == outputFormat.getVCharType())
        {
            pinyinStr = pinyinStr.replaceAll("u:", "ü");
        }

        if (ChinesePinyinCaseType.UPPERCASE == outputFormat.getCaseType())
        {
